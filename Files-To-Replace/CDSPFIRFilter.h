...
public:
		...
		const double* const AttenCorrs = NULL)
	{
		...
		while (CurObj != nullptr) {
			...
			{
				// Trovato il filtro in cache
				CurObj->RefCount++;

				if (PrevObj != nullptr) {
					// Sposta in testa alla lista
		...
		// Filtro non trovato. Se necessario, rimuovi l'ultimo filtro non utilizzato
		if (ObjCount >= R8B_FILTER_CACHE_MAX && LastObj != nullptr && LastObj->RefCount == 0) {
			// Rimuovi LastObj dalla lista
			if (PrevObj != nullptr && PrevObj->Next == LastObj) {
				PrevObj->Next = nullptr;
			}
			else if (Objects == LastObj) {
				Objects = nullptr;
			}
			delete LastObj;
			ObjCount--;
		...
		// Crea nuovo filtro
		CDSPFIRFilter* NewObj = new CDSPFIRFilter();
		NewObj->ReqNormFreq = ReqNormFreq;
		NewObj->ReqTransBand = ReqTransBand;
		NewObj->ReqAtten = ReqAtten;
		NewObj->ReqPhase = ReqPhase;
		NewObj->ReqGain = ReqGain;
		NewObj->RefCount = 1;

		NewObj->buildLPFilter(AttenCorrs);

		// Inserisci in testa
		NewObj->Next = Objects.unkeep();
		Objects = NewObj;
		ObjCount++;
