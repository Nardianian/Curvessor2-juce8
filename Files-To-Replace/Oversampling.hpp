...
public:
  ...
  public:
    ...
    int processBlock(InterleavedBuffer<Scalar> const& input, int numChannelsToUpsample, int numSamples)
    {
      if (firUpsampler) {
        ...
        return numSamples * static_cast<int>(int64_t{ 1 } << iirUpsampler->getOrder());
      }
    }
    ...
    {
      int const oversamplingRate = firUpsampler ? static_cast<int>(firUpsampler->getRate())
                                                : static_cast<int>(int64_t{ 1 } << iirUpsampler->getOrder());
    ...
    VecToVecUpsampler(OversamplingSettings const& settings)
    {
      if (settings.linearPhase) {
        ...
        firUpsampler->setRate(static_cast<int>(int64_t{ 1 } << settings.order));
    ...
    int getRate() const
    {
      ...
      if (iirUpsampler) {
        return static_cast<int>(int64_t{ 1 } << iirDownsampler->getOrder());
      }
    }
  };
  ...
  public:
    ...
    void prepareBuffers(int numSamples)
    {
      ...
      int const totalSamples = numSamples * oversamplingRate;
  ...
  public:
    ...
    VecToScalarDownsampler(OversamplingSettings const& settings)
    {
      ...
      prepareBuffers(settings.numSamplesPerBlock,
                     settings.numSamplesPerBlock * (static_cast<int>(int64_t{ 1 } << settings.order)));
    }
    ...
    {
      int const oversamplingRate = firDownsampler ? static_cast<int>(firDownsampler->getRate())
                                                  : static_cast<int>(int64_t{ 1 } << iirDownsampler->getOrder());
      if (firDownsampler) {
                      ...
                      int requiredSamples)
    {
      if (firDownsampler) {
        ...
        iirDownsampler->processBlock(
          input, static_cast<int>(int64_t{ requiredSamples } * (int64_t{ 1 } << iirDownsampler->getOrder())));
  ...
  public:
    ...
    ScalarToScalarDownsampler(OversamplingSettings const& settings)
    {
      if (settings.linearPhase) {
        firDownsampler = std::make_unique<TFirDownsampler<double>>(settings.numChannels, settings.firTransitionBand);
        firDownsampler->setRate(static_cast<int>(int64_t{ 1 } << settings.order));
        iirDownsampler = nullptr;
                      ...
                      int requiredSamples)
    {
      if (firDownsampler) {
        ...
        iirDownsampler->processBlock(
          iirInputBuffer, static_cast<int64_t>(requiredSamples) * (int64_t{ 1 } << iirDownsampler->getOrder()));
        iirDownsampler->getOutput().deinterleave(output, numChannelsToDownsample, requiredSamples);
      }
    }
  ...
  Oversampling(OversamplingSettings const& settings)
    ...
    , rate(static_cast<int>(int64_t(1) << settings.order))
  {
