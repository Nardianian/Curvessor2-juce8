...
CurvessorAudioProcessor::CurvessorAudioProcessor()
  ...
  , oversamplingSettings([this] {
    auto settings = OversamplingSettings{};
    settings.numScalarToVecUpsamplers = 3;
    settings.numVecToVecDownsamplers = 2;
    settings.numChannels = 2;
    settings.updateLatency = [this](int latency) {
  ...
  , oversampling(std::make_unique<Oversampling>(oversamplingSettings))
{
  ...
  oversamplingAttachments.emplace(parameters->oversampling,
                                    *parameters->apvts,
                                    this,
                                    &oversampling,
...
CurvessorAudioProcessor::Dsp::reset(Parameters& parameters)
{
  ...
  double const stereoLinkValue = 0.01 * parameters.stereoLink->get();
